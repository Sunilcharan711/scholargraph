...
def dense_statement(request: SearchRequest, spec: EmbeddingSpec, vector: list[float]) -> Select:
...
def dense_search(
    session: Session, provider: EmbeddingProvider, request: SearchRequest
) -> SearchResponse:
    start = perf_counter()
    ...
    rows = session.execute(dense_statement(request, spec, vector)).all()
    results = [
        SearchResult(
            ...
            similarity_score=max(-1.0, min(1.0, 1.0 - row.distance)),
        )
        for row in rows
    ]
    return SearchResponse(
        ...
        excluded_chunk_count=total - indexed,
